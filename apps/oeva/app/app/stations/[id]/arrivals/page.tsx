...
export default async function Arrivals({ params, searchParams }: { params: { id: string }, searchParams: { when?: string, products?: string[] } }): Promise<React.JSX.Element> {
    ...
    const arrivals = await client.arrivals(params.id, { duration: 1440, results: 40, when: when, products: buildProductsFilter(client, searchParams.products), remarks: true })
